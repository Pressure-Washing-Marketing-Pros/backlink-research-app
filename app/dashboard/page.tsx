...
export default function Dashboard() {
  ...
  const [sortBy, setSortBy] = useState<"created" | "payment_amount" | "score">("created");
  const [sortOrder, setSortOrder] = useState<"ASC" | "DESC">("DESC");
  const [currentPage, setCurrentPage] = useState(0);
  const limit = 50;
  const [tierModalId, setTierModalId] = useState<string | null>(null);
  ...
  const paymentTypes = useMemo(
    () => ["One-Time", "Per event", "Annual", "Monthly", "Recurring", "Free", "Unknown", "Other"],
    [],
  );
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
                ...
                <div>
                  ...
                  >
                    ...
                    <option value="payment_amount">Payment Amount (Low to High)</option>
                ...
                <>
                                  ...
                                  <div className="grid gap-4 md:grid-cols-2 text-sm">
                                    ...
                                    <div className="space-y-2">
                                      ...
                                      <div className="pt-4 border-t border-gray-200">
                                        <button
                                          onClick={() => {
                                            setTierModalId(opp.id);
                                            const existing = opp.sponsorship_tiers
                                              ? opp.sponsorship_tiers.split(", ").map((tier: string) => {
                                                  const match = tier.match(/^(.+?)\s*\$?(\d+)?\s*(\(includes link\))?$/);
                                                  return {
                                                    name: match ? match[1].trim() : tier,
                                                    price: match && match[2] ? `$${match[2]}` : "",
                                                    includesLink: /includes link/i.test(tier),
                                                  };
                                                })
                                              : [];
                                            setEditingTiers(existing);
                                          }}
                  ...
                  {tierModalId && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
                      <div className="rounded-lg bg-white shadow-xl max-w-2xl w-full mx-4">
                        <div className="border-b border-gray-200 px-6 py-4">
                          <h3 className="text-lg font-semibold text-gray-900">Edit Sponsorship Tiers</h3>
                        </div>
                        <div className="px-6 py-4">
                          ...
                          <button
                            ...
                            className="mb-4 text-blue-600 hover:text-blue-900 font-medium text-sm"
                          >
                            + Add Tier
                          </button>
                        </div>
                        <div className="border-t border-gray-200 px-6 py-4 flex gap-3 justify-end">
                          ...
                          <button
                            onClick={async () => {
                              ...
                              await fetch(`/api/opportunities/${tierModalId}`, {
                                method: "PATCH",
                                headers: { "Content-Type": "application/json" },
                                body: JSON.stringify({
                                  action: "updateTiers",
                                  sponsorship_tiers: formatted || "Unknown",
                                  cheapest_tier_with_link: cheapest
                                    ? `${cheapest.name} (${cheapest.price})`
                                    : "Unknown",
                                }),
                              });

                              const updated = await fetch(`/api/opportunities/${tierModalId}`);
                              if (updated.ok) {
                                const fresh: StoredOpportunity = await updated.json();
                                setOpportunities((prev) => prev.map((o) => (o.id === tierModalId ? fresh : o)));
                              }
                              setTierModalId(null);
                            }}
